...
昨晚折腾到深夜，终于把深度尝试了Grok Bot并上线了一个个人站点：[TopStream-AI精选资源](https://topstream-ai.vercel.app)。

说实话，动手前预期的上限不过是快速揉一个POC，再慢慢改，而且可能只能在Grok Bot环境内才能使用。但最后的结果**大大超出了我的预期**——从第一句“想做个展示站”的想法，到完成整个 Next.js 全栈应用、SQLite 数据库设计、公网稳定访问、打通 GitHub / Google / X 三方登录、搭建好管理后台，并顺手配置了几个自动化对话运维脚本，**整个过程前后只花了大概 2 小时，耗费了 SuperGrok 大约 30% 的用量额度**。

之前觉得Grok Bot比较鸡肋（只订阅了三个月，而且个人已经有其他各种agent工具在用）、实在没有特别刚需的场景和觉得有什么特别的地方。但高质量云主机 + 深度集成的Agent和模型能力 + 深度自动化能力大大提升了工作边界和想象空间，真的觉得Grok Bot模式的好用和便捷，可能后面真的会考虑长期订阅和继续深度使用。

---

## 成品先睹为快

站点本来就是开始想做个简单的个人精选资源收藏导航（毕竟平时自己浏览的工具网站、GitHub项目、硬核研报与学习笔记太多，没地方归整比较混乱），后续再逐渐扩展和丰富内容。核心是我自己日常高频翻阅的 AI 工具、开源项目、硬核研报与学习笔记的集合流：
...
### 步骤 2：云主机常驻 + Vercel 反代，公网入口稳定方案

应用跑在 Grok Bot 自带的那台持久 Linux 云主机上，数据库是轻便的本地 SQLite。这时候面临一个很现实的技术选择：**怎么向公网稳定暴露服务？** （note: 实际上还没有完全解决，但bot已经给了我完整解决方案，有时间了再继续完善）
...
站点打通了 GitHub、Google 和 X（Twitter）三个登录通道，下面是每个平台详细的后台设置路径与对应配置：
...
正好也可以利用Grok Bot完成这项工作，一大段收藏和笔记扔出去，自动整理分类发布：**把常用操作直接沉淀为 Bot 的技能**。

写了两个精简的 Node 自动化脚本，封装成了对话指令：
...
**Bot 已经从“帮你写代码的工具”变成了“帮你打理站点的专属助理”。**
...
## 个人体会：为什么 Grok Bot 远超我的预期？

最大的感触不是“AI 帮我写代码”，而是：**端到端完成一些自己以前觉得很麻烦的事情现在变得极其简单了，甚至很多完全不熟悉不了解的领域也能快速入手**。

### 1. “带主机环境”对纯聊天模型是降维打击
Grok Bot 自带一台完整的持久云主机。终端命令它自己执行、包依赖它自己装、报错它自己看日志重试，连测试连通性都是它自己用 `curl` 跑完再向我汇报。这种**“能动手的 AI”**和**“动动嘴皮子的 AI”**完全不是一个物种。
...
- **可视化云端环境**：通过图形化的云端环境，可以看到 AI 执行过程，也可以随时接管

### 3. 可探索的场景极其丰富，这仅仅是个开始
这个导航站只是一个最基础的原型验证。有了这套“持久主机 + 数据库 + 执行力 + 对话技能”的组合，未来能玩出的花样实在太多了：
...
是一个随时听候调遣、不仅有脑子还有手脚的全能搭档，还是相当有性价比的。

---

## 【2026-09-04 追更】一次真实的 502 故障与架构验证

> **记录时间**：2026-09-04  
> **触发背景**：刚上线没多久，这套架构就遇到了一次真实的“小插曲”——站点突然打不开了。正好借这次实际故障排查，补充一下运行中的坑点和后续的长效运维思路。


### 1. 今天到底发生了什么？

外部访问的公开地址**完全没变**，依旧是：https://topstream-ai.vercel.app  
但访问时直接撞上了 Vercel 的报错：`502 DNS_HOSTNAME_NOT_FOUND`。

整个链路其实是这样串起来的：  
`外部访问 → Vercel 反代 → Cloudflare 临时隧道（*.trycloudflare.com）→ 主机本地 8787 端口`

排查发现，主机里的 Next.js 进程其实一直正常跑着，但 Cloudflare 免费的临时穿透域名（之前分配的 `ecology-tampa-surgical-salt.trycloudflare.com`）DNS 解析突然失效了，导致 Vercel 找不到上游源站。

**紧急恢复操作：**  
重新拉起一个临时隧道拿到新域名 → 更新 `topstream-proxy/vercel.json` 里的 rewrite 指向 → 运行 `vercel --prod` 重新发布。站点几分钟内顺利恢复。

### 2. OAuth 登录和平台配置要不要改？

**完全不用改。**  
这也是在步骤 2 里坚持用 Vercel 做前门反代最爽的地方：  
- `AUTH_URL` 以及 GitHub / Google / X 的回调地址，自始至终都认准 `https://topstream-ai.vercel.app`。
- 临时穿透域名挂了换新，只影响 Vercel 这一层的上游代理配置。**千万别去动各大平台的 OAuth 开发者后台，也不用改线上环境变量。** 对外部用户和登录系统来说，底层隧道的更换是完全透明的。

### 3. 目前做好的自动化兜底

虽然手动换域名改配置也就两三分钟，但人总不能一直肉眼盯着。所以顺手做了一套定时自愈机制：

- **自愈脚本**：`topstream-proxy/heal-origin.sh`（自动探测服务健康 → 异常时自动重建隧道 → 替换 `vercel.json` 里的目标地址 → 自动触发部署）。
- **定时巡检**：配置了「TopStream 入口自愈」任务，每天 8:00–23:30 每隔 30 分钟探活一次。平时一切正常时不发通知打扰，只有在触发修复或修复失败时才会推送提醒。

不过必须清楚：**这只能尽量缩短掉线后的恢复时间，解决不了免费临时隧道本身就不稳定的底子。**

### 4. 长期更稳的建议（按优先级）

如果想彻底告别偶尔掉线的焦虑，后续有几个更靠谱的演进方案：

1. **Cloudflare Named Tunnel（命名隧道，首选）**  
   直接在 Cloudflare 上建固定名字的隧道（或者绑定自己的域名），Vercel 里的 rewrite 规则只写一次就永久生效。这种官方有 SLA 保障，机器重启也不会换域名。只需要去 Cloudflare 控制台做一次性配置。
2. **自有域名直连**  
   例如用 `app.你的域名` 直接挂 Named Tunnel 或固定的云服务器；Vercel 可以退居二线做边缘节点或备用。OAuth 回调统一迁到自有域名（这次需要在各平台改一次回调地址，但改完一劳永逸）。
3. **继续用临时隧道 + 脚本自愈（现状）**  
   零成本、现成可用，对个人折腾项目完全够用。缺点是每次掉线到脚本探活并重新部署之间，难免会有几分钟的访问空窗，适合临时过渡，不适合作为高可靠的长期依赖。

**一句话总结：**  
**对外永远固定死 `vercel.app`（或以后换成自有域名）；对内尽快升级成 Named Tunnel 命名隧道，把定时自愈脚本留作兜底保险。**
