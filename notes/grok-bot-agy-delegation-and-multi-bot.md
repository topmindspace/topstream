...
## 二、装进云主机：Bot 自己装，人只管授权

Grok Bot 给每个 Bot 都配了持久的 Linux 环境，这一点是关键。装一次、登一次，之后就是常驻的。

整个安装配置过程，我没敲一条命令。大管家 bot 先装 `antigravity-cli`，装好后发现是无头环境，就生成一个授权链接；我把桌面接过来完成 Google 登录，授权码贴回去，它负责收尾。

然后它自己跑无头冒烟验证：返回 `AGY_OK`，模型列表里确认有 Gemini 3.8 Flash / 3.1 Pro，最后把委派用法沉淀成一个 `ai-delegate` 技能。我全程只出现在“授权”那一步。

![Bot 自主安装 agy](../assets/images/agy-bot-self-install.png)
...
接下来我想试的是把闲置的低成本订阅激活起来。比如装 `pi` 这类开源 agent CLI，配上 MiniMax 的 token 套餐；或者把手里已有的其他低价 token plan 接进来。

这些模型能力上比旗舰差一截，但便宜量大。而调度层拆开之后，恰好有一类活不需要旗舰：写脚本、跑批、格式转换、批量整理和归类。这类活用便宜模型绰绰有余，派给旗舰反而浪费。

判断标准和 agy 一样：只要接口是“丢进去一条自包含指令、拿回来一个结构化结果”，就能挂进这套调度里。以后哪家更划算就换哪家，调度层不用动。

原则也从“哪个强用哪个”变成**“够用且便宜”**——几份订阅各管一摊，谁闲着用谁。
